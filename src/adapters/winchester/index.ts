 ...
 * 
 * Production-quality adapter using Playwright browser automation for Winchester's React SPA.
 * Winchester uses a JavaScript-rendered interface requiring browser execution.
 * 
 * SELECTORS_VALIDATED: false (pending manual verification)
 * THIRD_PARTY_RISK: none
 * 
 * @module adapters/winchester
 */
...
import type { WinchesterAddress, WinchesterHtmlData } from './types.js';
import {
  validatePostcode,
...
const WINCHESTER_LOOKUP_PATH = '/icollectionday/';

export class WinchesterAdapter extends BrowserAdapter implements CouncilAdapter {
  readonly councilId = 'winchester';
  
  constructor() {
    super({
      allowedDomains: ['winchester.gov.uk'],
  ...
  async discoverCapabilities(): Promise<CouncilCapabilities> {
    return {
      ...
      councilWebsite: WINCHESTER_BASE_URL,
      ...
      limitations: [
        'Requires browser automation (React SPA)',
  ...
  async resolveAddresses(input: PropertyLookupInput): Promise<AddressCandidateResult> {
    ...
    // Check kill switch
    if (process.env.ADAPTER_KILL_SWITCH_WINCHESTER === 'true') {
      return this.failureResult(
        metadata,
    ...
    if (!SELECTORS_VALIDATED) {
      console.warn('[WINCHESTER] Selectors not yet validated in production — schema drift risk');
    }
    
    try {
      const result = await this.executeBrowserTask<AddressCandidate[]>(async (page) => {
        const lookupUrl = `${WINCHESTER_BASE_URL}${WINCHESTER_LOOKUP_PATH}`;
        const navResult = await navigateToLookupPage(page, {
          baseUrl: lookupUrl,
          expectedDomain: 'winchester.gov.uk',
        });
        ...
        if (!validateOnDomain(page, 'winchester.gov.uk')) {
          throw new Error('Redirected off winchester.gov.uk domain');
        }
  ...
  async securityProfile(): Promise<AdapterSecurityProfile> {
    return {
      ...
      externalDomains: ['winchester.gov.uk'],
      handlesCredentials: false,
      securityConcerns: [
        'Browser automation required (React SPA)',
        'JavaScript execution from winchester.gov.uk',
        'Selectors not validated — schema drift risk',
        'React application updates may break adapter',
      ],
  ...
  private async extractAddresses(page: Page): Promise<WinchesterAddress[]> {
    const addresses: WinchesterAddress[] = [];
  ...
  ): Promise<WinchesterHtmlData | null> {
    const result = await this.executeBrowserTask<WinchesterHtmlData>(async (page) => {
      const lookupUrl = `${WINCHESTER_BASE_URL}${WINCHESTER_LOOKUP_PATH}`;
      const navResult = await navigateToLookupPage(page, {
        baseUrl: lookupUrl,
        expectedDomain: 'winchester.gov.uk',
  ...
  private async parseCollectionSchedule(page: Page): Promise<WinchesterHtmlData> {
    const warnings: string[] = [];
    ...
    for (const selector of collectionSelectors) {
      ...
      for (const row of rows) {
        ...
        const serviceMatch = text.match(/(Refuse|Recycling|Food|Garden|Glass|Waste)\s*(bin)?/i);
    ...
    if (collections.length === 0) {
      warnings.push('No collection data found — React component structure may have changed');
    }
