...
import type { HavantAddress, HavantHtmlData } from './types.js';
import {
  validatePostcode,
...
const LOOKUP_PATH = '/bin-collections';

// Set to false initially - requires validation against live site
const SELECTORS_VALIDATED = false;

export class HavantAdapter extends BrowserAdapter implements CouncilAdapter {
  readonly councilId = 'havant';
  
  constructor() {
    super({
      allowedDomains: ['havant.gov.uk'],
  ...
  async discoverCapabilities(): Promise<CouncilCapabilities> {
    return {
      ...
      councilName: 'Havant Borough Council',
      councilWebsite: 'https://www.havant.gov.uk',
  ...
  async resolveAddresses(input: PropertyLookupInput): Promise<AddressCandidateResult> {
    ...
    // Check kill switch
    if (process.env.ADAPTER_KILL_SWITCH_HAVANT_DEANE === 'true') {
      return this.failureResult(
        metadata,
    ...
    try {
      const result = await this.executeBrowserTask<AddressCandidate[]>(async (page) => {
        const lookupUrl = `${HAVANT_URL}${LOOKUP_PATH}`;
  ...
  async verifyHealth(): Promise<AdapterHealth> {
    ...
    try {
      const testPostcode = 'PO9 1AA'; // Havant area test postcode
   ...
   */
  private async extractAddresses(page: Page): Promise<HavantAddress[]> {
    const addresses: HavantAddress[] = [];
  ...
  ): Promise<HavantHtmlData | null> {
    const result = await this.executeBrowserTask<HavantHtmlData>(async (page) => {
      const lookupUrl = `${HAVANT_URL}${LOOKUP_PATH}`;
   ...
   * TODO: Validate these selectors against live Basingstoke site.
   */
  private async parseCollectionSchedule(page: Page): Promise<HavantHtmlData> {
