/**
 * Havant & Deane Borough Council - Response Parser
 * 
 * Parses untrusted upstream responses (HTML or JSON) into normalised events.
 * 
 * @module adapters/Havant-deane/parser
 */
...
/**
 * Map Havant service names to canonical ServiceType enum.
 */
...
/**
 * Parse individual Havant collection into CollectionEvent.
 */
export function parseCollectionEvent(
  raw: HavantCollection,
  index: number
  ...
  return {
    eventId: `Havant-deane-${collectionDate}-${serviceType}-${index}`,
    serviceId: `Havant-deane-${serviceType}`,
    serviceType,
    collectionDate,
...
/**
 * Parse Havant response into collection events.
 */
export function parseCollectionEvents(
  response: HavantRawResponse | HavantHtmlData
): CollectionEvent[] {
...
/**
 * Parse address candidates from Havant address lookup.
 */
export function parseAddressCandidates(
  addresses: HavantAddress[],
  postcode: string
): AddressCandidate[] {
