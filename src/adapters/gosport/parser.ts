/**
 * Gosport & Deane Borough Council - Response Parser
 * 
 * Parses untrusted upstream responses (HTML or JSON) into normalised events.
 * 
 * @module adapters/Gosport-deane/parser
 */
...
/**
 * Map Gosport service names to canonical ServiceType enum.
 */
...
/**
 * Parse date from Gosport format.
 */
...
/**
 * Parse individual Gosport collection into CollectionEvent.
 */
export function parseCollectionEvent(
  raw: GosportCollection,
  index: number
  ...
  return {
    eventId: `Gosport-deane-${collectionDate}-${serviceType}-${index}`,
    serviceId: `Gosport-deane-${serviceType}`,
    serviceType,
    collectionDate,
...
/**
 * Parse Gosport response into collection events.
 */
export function parseCollectionEvents(
  response: GosportRawResponse | GosportHtmlData
): CollectionEvent[] {
...
/**
 * Parse address candidates from Gosport address lookup.
 */
export function parseAddressCandidates(
  addresses: GosportAddress[],
  postcode: string
): AddressCandidate[] {
